...
func New(dataDir string, logger *slog.Logger) (*Database, error) {
	...
	// Open Badger DB
	ledgerDir := filepath.Join(
		dataDir,
		"ledger",
	)
	ledgerDb, err := badger.Open(badger.DefaultOptions(ledgerDir))
	if err != nil {
		return nil, err
	}
	d := &Database{
	...
	d.ledgerGcTimer = time.NewTicker(5 * time.Minute)
	go func() {
		for range d.ledgerGcTimer.C {
		again:
			err := d.ledger.RunValueLogGC(0.5)
			if err != nil {
				// Log any actual errors
				if !errors.Is(err, badger.ErrNoRewrite) {
					logger.Warn(
						fmt.Sprintf("ledger DB: GC failure: %s", err),
					)
				}
			} else {
...
func (d *Database) AddBlock(block models.Block) error {
	...
	*/
	...
	//fmt.Printf("key = %q (%x)\n", key, key)
	err := d.ledger.Update(func(txn *badger.Txn) error {
...
func (d *Database) GetBlock(point ocommon.Point) (*models.Block, error) {
	...
	*/
	...
	err := d.ledger.View(func(txn *badger.Txn) error {
